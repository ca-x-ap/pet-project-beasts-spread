...
def prepare_data():
	...
	if len_sys_argv == 0 or len_sys_argv == 1:
		...
		print(f'(You can run app as "app.py years_count({ years })(need write) start_count({ start_count }) children_count({ children_count }) min_children_count({ min_children_count }) live_time({ live_time }) min_live_time({ min_live_time })" )')
	if len_sys_argv >= 1:
